<template>
  <div class="h-screen w-full relative font-sans selection:bg-indigo-500/30 flex flex-col overflow-hidden">
    ...
    <header class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 h-14">
      <div class="absolute inset-0 bg-white/95 border-b border-zinc-200 shadow-2xs"></div>
      
      <div class="relative max-w-[1600px] mx-auto px-4 sm:px-6 h-full">
        <div class="flex justify-between items-center h-full">
          
          <div class="flex items-center gap-3 cursor-pointer group" @click="$router.push('/')">
            <div class="relative w-8 h-8 rounded-lg bg-zinc-900 shadow-md shadow-zinc-900/10 flex items-center justify-center overflow-hidden transition-transform group-hover:scale-105 duration-300">
              ...
              <div v-else class="text-white font-semibold text-base font-mono">M</div>
            </div>
            <div class="flex flex-col">
              <span class="text-sm font-bold text-zinc-900 tracking-tight leading-none">MagicForge</span>
              <span class="text-[8px] font-bold text-zinc-400 uppercase tracking-widest mt-0.5">Platform</span>
            </div>
          </div>
          ...
          <nav class="hidden lg:flex items-center gap-1">
            <router-link 
              ...
              :key="item.path" 
              :to="item.path" 
              class="relative px-3 py-1.5 rounded-sm text-xs font-semibold transition-all duration-200 ease-out group flex items-center gap-1.5" 
              :class="$route.path === item.path ? 'text-zinc-900 bg-zinc-100' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50'"
            >
              <component 
                :is="item.icon" 
          ...
          <div class="lg:hidden flex items-center">
            <button @click="mobileMenuOpen = !mobileMenuOpen" class="relative w-9 h-9 flex items-center justify-center rounded-sm hover:bg-zinc-100 transition-colors cursor-pointer">
              <div class="flex flex-col justify-center items-center gap-[4px] w-4 h-4 relative">
                <span class="w-4 h-[1.5px] bg-zinc-700 rounded-full transition-all duration-300 absolute" :class="mobileMenuOpen ? 'rotate-45' : '-translate-y-[5px]'"></span>
                <span class="w-4 h-[1.5px] bg-zinc-700 rounded-full transition-all duration-300 absolute" :class="mobileMenuOpen ? 'opacity-0' : 'opacity-100'"></span>
                <span class="w-4 h-[1.5px] bg-zinc-700 rounded-full transition-all duration-300 absolute" :class="mobileMenuOpen ? '-rotate-45' : 'translate-y-[5px]'"></span>
              </div>
            </button>
          </div>
      ...
      <div class="lg:hidden absolute top-full left-0 w-full overflow-hidden transition-all duration-500" :class="mobileMenuOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'">
        <div class="px-4 pb-6 bg-white border-b border-zinc-200 shadow-xl">
          <div class="flex flex-col gap-1 mt-2">
            <router-link 
              ...
              class="flex items-center gap-3 px-3.5 py-2.5 rounded-sm transition-all duration-200" 
              :class="$route.path === item.path ? 'bg-zinc-100 text-zinc-900' : 'text-zinc-500 hover:bg-zinc-50'"
            >
              <component :is="item.icon" class="w-4 h-4" :class="$route.path === item.path ? 'text-zinc-900' : 'text-zinc-400'" />
...
<footer class="relative z-1 border-t border-zinc-200 bg-white" v-if="$route.path !== '/' && $route.path !== '/api-demo'">
  <div class="max-w-[1600px] mx-auto px-4 sm:px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-3">
    ...
    <p class="text-xs font-medium text-zinc-400 tracking-wide text-center">
      © 2026 MagicForge Platform. <span class="hidden md:inline text-zinc-300 mx-1.5">|</span> <span class="block md:inline mt-1 md:mt-0">Design for Future.</span>
    </p>

    <div class="flex items-center gap-4 text-zinc-400">
      <a href="#" class="hover:text-zinc-900 transition-colors duration-200 cursor-pointer">
