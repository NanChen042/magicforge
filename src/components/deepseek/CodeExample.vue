<template>
  <div class="max-w-7xl mx-auto mb-12 px-4">
    ...
    <!-- 1. 窗口头部：模拟 IDE 标题栏 -->
    <div class="relative bg-zinc-900 rounded-t-sm border border-zinc-800 border-b-0 px-4 py-3 flex items-center justify-between">
      <!-- 窗口控制点 -->
    ...
    <!-- 2. 代码区域 -->
    <div class="relative group bg-[#0d0d0d] rounded-b-sm border border-zinc-800 overflow-hidden">
      
      <!-- 复制按钮 (悬浮) -->
      <button 
        @click="copyCode"
        class="absolute top-4 right-4 p-2 rounded-sm bg-zinc-800/50 text-zinc-400 hover:text-white hover:bg-zinc-700 backdrop-blur-sm transition-all opacity-0 group-hover:opacity-100 border border-zinc-700/50"
        title="Copy Code"
      >
