...
<template>
  <div class="space-y-5 font-sans text-zinc-800">
    ...
    <!-- 场景描述卡片 -->
    <div class="relative overflow-hidden bg-white rounded-md p-5 border border-zinc-200 shadow-2xs">
       <div class="flex items-start gap-4">
          <div class="flex-shrink-0">
             <div class="w-10 h-10 bg-blue-50 rounded-sm flex items-center justify-center text-blue-600 border border-blue-100">
          ...
          <div class="flex-1 space-y-1.5">
             <div class="text-blue-600 font-bold text-xs tracking-wide flex items-center gap-1.5">
                <span class="w-1.5 h-1.5 rounded-full bg-blue-600"></span>
                当前场景概览
             </div>
             
             <!-- 场景文本 -->
             <div v-if="currentScene?.description" class="text-zinc-700 leading-relaxed text-sm animate-fade-in text-justify">
                {{ currentScene.description }}
             </div>
             
             <!-- Loading -->
             <div v-else-if="isGenerating" class="flex items-center gap-2 text-zinc-400 italic text-sm">
                <span class="animate-pulse">正在生成场景环境...</span>
             </div>
             ...
             <div v-else-if="aiErrorMessage" class="flex items-center gap-3 mt-2">
                ...
                <button @click="emit('retryCurrentScene')" class="px-2.5 py-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 rounded-sm text-xs transition-colors font-medium">
                   重试
                </button>
             </div>
          </div>
    ...
    <div class="space-y-3">
      ...
      <!-- Tab 切换 -->
      <div class="bg-zinc-100 p-1 rounded-sm flex gap-1 border border-zinc-200/60">
        <!-- Tab 1: 对话内容 -->
        <button
          @click="emit('switchTab', 'dialog')"
          class="flex-1 py-2 rounded-sm text-xs font-semibold transition-all duration-200 flex items-center justify-center gap-1.5 cursor-pointer"
        ...
        <button
          ...
          class="flex-1 py-2 rounded-sm text-xs font-semibold transition-all duration-200 flex items-center justify-center gap-1.5 relative cursor-pointer"
        ...
        <transition name="fade" mode="out-in">
          <div v-if="activeTab === 'dialog'" key="dialog" class="bg-white rounded-md p-5 border border-zinc-200 shadow-2xs">
             <div class="flex gap-4 items-start">
                <div class="w-8 h-8 rounded-sm bg-blue-50 text-blue-600 flex items-center justify-center border border-blue-100 shrink-0">
                ...
                <div class="flex-1 pt-0.5 min-w-0">
                   <div v-if="isGenerating && !currentDialogStream" class="min-h-[100px] flex flex-col items-center justify-center gap-2 bg-zinc-50 rounded-sm border border-zinc-200 border-dashed">
                      ...
                      <span class="text-xs text-zinc-400">AI 正在生成剧情对话...</span>
                   </div>

                   <div v-else-if="currentDialogStream || currentScene?.dialog" class="min-h-[60px] text-zinc-800 leading-relaxed text-sm whitespace-pre-line animate-fade-in">
             ...
             <div class="flex gap-4 items-start">
                ...
                <div class="flex-1 pt-0.5 min-w-0">
                   <div v-if="isGenerating && !reasoningContent" class="h-[240px] flex flex-col items-center justify-center gap-3 bg-zinc-50 rounded-sm border border-zinc-200 border-dashed">
                      ...
                      <span class="text-xs text-zinc-400">AI 正在深度思考逻辑分支...</span>
                   </div>
                   
                   <div v-else class="h-[240px] overflow-y-auto custom-scrollbar font-mono text-xs text-zinc-600 leading-relaxed bg-zinc-50 rounded-sm p-4 border border-zinc-200">
                      <div v-if="reasoningContent" class="whitespace-pre-wrap">{{ reasoningContent }}</div>
                      <div v-else class="h-full flex items-center justify-center text-zinc-400 italic">
                        暂无思考过程数据...
                      </div>
                   </div>
                </div>
