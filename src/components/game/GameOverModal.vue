    ...
    >
      ...
      >
        ...
        >
          <div
            v-if="show"
            class="relative w-full max-w-sm overflow-hidden rounded-md border border-zinc-200 bg-white shadow-xl p-6 text-center"
          >
            <!-- Header Icon -->
            <div 
            ...
            <h2 class="text-lg font-bold text-zinc-900 mb-2">{{ title }}</h2>
            <p class="text-zinc-600 text-xs leading-relaxed mb-6 whitespace-pre-line">
              {{ message }}
            </p>

            <button
              @click="emit('close')"
              class="w-full py-2.5 rounded-sm font-semibold text-xs text-white transition-all cursor-pointer shadow-2xs"
