import{C as g}from"./api-BmTD3KbZ.js";class f{apiKey;baseURL;model;temperature;maxTokens;topP;topK;frequencyPenalty;presencePenalty;sceneHistory=[];usedSpecialEvents=new Set;sceneTypes=["课堂学习","游戏训练","排位赛","直播现场","考试现场","社交互动","家庭生活","社团活动","比赛解说","粉丝互动","感情困惑","人生思考"];specialEvents=["炸麦警告（情绪激动导致麦克风爆音）","躺赢时刻（队友carry）","连招练习（剑魔技能连招）","偶遇李雪（暗恋对象）","王老师突袭（被抓打游戏）","高考倒计时（压力事件）","粉丝认出（身份暴露）","网络卡顿（比赛掉线）","家长谈话（成绩下滑）","赛事解说（受邀解说）","感情困惑（对话选择）","人生思考（价值观碰撞）"];characterBackground=`角色背景：
- 姓名：小明（游戏ID：最强剑魔）
- 身份：高三学生，知名游戏主播
...
- 性格特点：
  ...
  2. 直播时经常情绪激动导致麦克风爆音
  3. 对游戏有着极强的执着
  4. 对感情和生活有独特见解`;constructor(e){this.apiKey=e.apiKey,this.baseURL=e.baseURL,this.model=e.model||"deepseek-r1",this.temperature=e.temperature??.8,this.maxTokens=e.maxTokens??2e3,this.topP=e.topP,this.topK=e.topK,this.frequencyPenalty=e.frequencyPenalty,this.presencePenalty=e.presencePenalty}async chatCompletion(e){try{const t=this.ensureCorrectApiUrl(this.baseURL),{messages:n,stream:r=!1,signal:s,...c}=e,o={model:this.model,messages:n,temperature:this.temperature,max_tokens:this.maxTokens,stream:r,...c};this.topP!==void 0&&(o.top_p=this.topP),this.topK!==void 0&&(o.top_k=this.topK>=1&&this.topK<=100?Math.round(this.topK):-1),this.frequencyPenalty!==void 0&&(o.frequency_penalty=this.frequencyPenalty),this.presencePenalty!==void 0&&(o.presence_penalty=this.presencePenalty);const i=await fetch(t,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.apiKey}`},body:JSON.stringify(o),signal:e.signal});if(!i.ok){const a=await i.json().catch(()=>null);throw new Error(`API请求失败: ${i.status} ${i.statusText}
${a?JSON.stringify(a):""}`)}return e.stream?this.handleStream(i):i.json()}catch(t){throw console.error("API请求失败:",t),t}}get completions(){return{create:e=>this.chatCompletion(e)}}get chat(){const e=this;return{completions:{create:t=>e.chatCompletion(t)}}}ensureCorrectApiUrl(e){return e.includes("/chat/completions")?e:`${e.endsWith("/")?e.slice(0,-1):e}/chat/completions`}async handleStream(e){return{[Symbol.asyncIterator](){const n=e.body.getReader(),r=new TextDecoder;let s="";const c=[],o=()=>{const i=[];let a=!1;const l=/data:\s*(\[DONE\]|\{[^]*?\})(?=\s*(?:data:|$))/g;let u,h=0;for(;(u=l.exec(s))!==null;){const p=u[1].trim();if(h=l.lastIndex,p==="[DONE]"){a=!0;break}try{const d=JSON.parse(p);i.push(d)}catch{console.warn("SSE JSON 解析失败，可能是不完整数据:",p.substring(0,100))}}return h>0&&(s=s.substring(h)),{chunks:i,isDone:a}};return{async next(){try{if(c.length>0)return{value:c.shift(),done:!1};const{done:i,value:a}=await n.read();if(i){if(s.trim().length>0){const{chunks:h,isDone:p}=o();if(h.length>0)return c.push(...h.slice(1)),{value:h[0],done:!1}}return{done:!0,value:void 0}}s+=r.decode(a,{stream:!0});const{chunks:l,isDone:u}=o();return u?{done:!0,value:void 0}:l.length>0?(c.push(...l.slice(1)),{value:l[0],done:!1}):this.next()}catch(i){return console.error("流式处理出错",i),{done:!0,value:void 0}}},async return(){return await n.cancel(),{done:!0,value:void 0}}}}}}async createStreamingResponse(e,t){const n=await fetch(e,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.apiKey}`},body:JSON.stringify(t)});if(!n.ok){const r=await n.text();throw new Error(`API请求失败: ${n.status} - ${r}`)}return this.handleStream(n)}async generateInitialScene(){const e=this.sceneTypes[Math.floor(Math.random()*this.sceneTypes.length)],t=Math.random()>.7?this.specialEvents[Math.floor(Math.random()*this.specialEvents.length)]:void 0,n=`请以第一人称的视角，扮演小明（最强剑魔）生成游戏开场场景。
